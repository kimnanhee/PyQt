...
class draw_graph(QtWidgets.QMainWindow, Ui_MainWindow):
	...
	def __init__(self, parent=None): # 설정 함수
		...
		self.graphicsView_temp.setBackground('#FFFFFF') # 배경 색깔 설정
		self.graphicsView_temp.showGrid(x=True, y=True) # 그리드 설정
		self.graphicsView_temp.setRange(xRange=[2, 20]) # x축 간격 설정
		self.graphicsView_humi.setBackground('#FFFFFF')
		self.graphicsView_humi.showGrid(x=True, y=True)
		self.graphicsView_humi.setRange(xRange=[2, 20])
	
	def uiUpdater(self):
		self.graphicsView_temp.clear() # 그래프 지우기
		self.graphicsView_humi.clear()
		self.graphicsView_temp.plot(temp_list, pen=pg.mkPen('r', width=2), # 온도
			style=QtCore.Qt.DashLine, symbol=('o'), symbolBrush='r')
		self.graphicsView_humi.plot(humi_list, pen=pg.mkPen('b', width=3), # 습도
